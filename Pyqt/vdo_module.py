...
from detector import Detector,get_Img
...
class VdoModule(QtWidgets.QMainWindow, Ui_TOTAL):
    ...
    def ChooseVideo(self):
        ...
        if fileName_choose == "":
            ...
            self.label_7.show()
            # 按钮绑定
            self.reselect_vdo.clicked.connect(self.Reselect_vdo)
            # 视频第一帧
            try:
                _thros = self.Slider_2.value()*0.01
                self.threshold_vdo.setText(f"%.2f" % _thros)
                get = get_Img(video_path=["{}".format(Vdo_fileName)])
                vdo_frame = get.get()
                for _ in vdo_frame:
                    vdo_frame = _
    ...
    def Submit_vdo(self):
        ...
        # 按钮解绑
        self.reselect_vdo.disconnect()
        # 按钮绑定
        self.reselect_vdo.clicked.connect(self.make_vdooff)
        flag = 0
        Det = Detector(video_path="{}".format(Vdo_fileName), track=True)
        Det_vdo = Det.video_detect(threshold=threshold)
        self.vdo_thread.start()

    # ROI识别按钮
    def ROI_submit(self):
        global threshold, flag, Det_vdo, vdo_flag, Det
        vdo_flag = 0
        self.submit_vdo.hide()
        self.vdo.hide()
        self.Slider_2.hide()
    ...
    def thread_unlock(self):
        global lock_flag, value, Det_vdo, flag, Det
    ...
    def id_box(self):
        ...
        if lens < 0:
            ...
            for i in range(abs(lens)):
                ...
                if locate != -1:
                    if str(ls4[i]) == "0":
                        self.idBox.addItem("所有行人")
                    self.idBox.insertItem(locate, str(ls4[i]))
                else:
                    if str(ls4[i]) == "0":
                        self.idBox.addItem("所有行人")
                    else:
...
class HistoryWindow(QDialog, Ui_History):
    ...
    def winshow(self):
        ...
        try:
            ...
            info1 = QMessageBox()
            QMessageBox.information(info1, "信息", "历史记录为空", QMessageBox.Ok, QMessageBox.Ok)
            return

        # 判空
        if not self.lens:
            info2 = QMessageBox()
            QMessageBox.information(info2, "信息", "历史记录为空", QMessageBox.Ok, QMessageBox.Ok)
            return
    ...
    def delbtn(self):
        ...
        if ans == QMessageBox.Yes:
            ...
            try:
                os.remove(filedir)
                self.data_base.delete((time,))     # 传过去的数据必须是元组
